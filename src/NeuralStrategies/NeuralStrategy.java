 ...
 * @author Olivier
 */
public class NeuralStrategy  implements IStrategy {
   ...
   private int MaxEpochs = 500;
    ...
    public void onStart(IContext context) throws JFException {
        ...
        Printer.println("-- Dukascopy with Encog --");
        Printer.println("");
      ...
      //  Calendar StartDate = Calendar.getInstance(); 
        try {
          ...
          //   barList=  JForexContext.getHistory().getBars(Instrument.EURUSD, Period.ONE_HOUR, OfferSide.BID , Filter.WEEKENDS , 500,StartDate.getTimeInMillis(),1000);
            ...
            Logger.getLogger(NeuralStrategy.class.getName()).log(Level.SEVERE, null, ex);
        }
...
dateFormat.setTimeZone(TimeZone.getTimeZone("GMT"));
    ...
    List<IBar> bars = JForexContext.getHistory().getBars(Instrument.EURUSD, Period.ONE_HOUR, OfferSide.ASK, dateFrom.getTime(), dateTo.getTime());
  ...
  /*      if (period != Period.TEN_SECS) return;      // only run every 10 sec.
        ...
        String symbol = JForexAccount.getAccount().getCurrency().getSymbol();
